...
When two features have a combined effect, this is known as an **interaction**.  If it is thought that two variables interact, this hypothesis can be tested by adding their interaction to the model.  Interaction effects are specified uing the R formula syntax.  To have the obesity indicator _bmi30_ and the smoking indicator _smoker_ interact, write the formula in the form ```expenses ~ bmi30*smoker```.
